...
void simonKey64_128(int SIMON_M,int SIMON_T,uint32_t *k)
{
  uint32_t tmp,tmp2;
    ...
    {  
        ...
        k[i] = ~k[i-SIMON_M] ^ tmp ^ z[3][i-SIMON_M] ^ 3;
        //Serial.println(k[i],HEX);
    }
}

void simonEnc64_128(int SIMON_M,int SIMON_T,uint32_t *key,uint64_t cword)
{
  uint32_t crypt[1];
  u32 tmp;
  crypt[0]= cword >>32;
