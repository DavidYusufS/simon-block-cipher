...
void simonKey128_192(int SIMON_M,int SIMON_T,uint64_t *k)
{
  uint64_t tmp,tmp2;
...
//  Serial.println();
    ...
    {  
        ...
        k[i] = ~k[i-SIMON_M] ^ tmp ^ z[3][(i-SIMON_M)%62] ^ 3;
//        printLLNumber(k[i],16);
//        Serial.println();
    }
}

void simonEnc128_192(int SIMON_M,int SIMON_T,uint64_t *key,uint64_t *cword)
{
  uint64_t crypt[1];
  uint64_t tmp;
  crypt[0]= cword[0];
